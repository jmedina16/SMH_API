...
class Ppv_user_model extends CI_Model {

    public function __construct() {
        // Open the correct DB connection
        $this->accounts = $this->load->database('ppv', TRUE);
    ...
    public function get_user_list($pid, $ks, $start, $length, $search, $draw) {
        ...
        if ($valid['success']) {
            $columns = array('status', 'user_id', 'first_name', 'last_name', 'email', 'restriction', 'created_at', 'updated_at');
...
//            }
            ...
            foreach ($users_res as $user) {
                ...
                $orders = $valid["pid"] . "," . $user["user_id"] . ",\"" . $user["first_name"] . " " . $user["last_name"] . "\"";
                $restriction_data = $valid["pid"] . ",\"" . $user["email"] . "\"," . $user["restriction"];
                ...
                if ($user['logged_in']) {
                    ...
                    $destroy_session = "<li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.destroySession(" . $session_data . ");'>Destroy Session</a></li>";
                }
                ...
                $actions = "<span class='dropdown header'>
                                    <div class='btn-group'>
                                        ...
                                        <ul class='dropdown-menu' id='menu' role='menu' aria-labelledby='dropdownMenu'> 
                                            <li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.editUser(" . $user_data . ");'>User</a></li>" .
                                            "<li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.editUserDetails(" . $user["user_id"] . ",\"" . $user['user_details'] . "\");'>Additional Details</a></li>" .
                                            $destroy_session .
                                            "<li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.userOrders(" . $orders . ");'>View Orders</a></li>
                                            <li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.userRestriction(" . $restriction_data . ");'>Restriction</a></li>
                                            <li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.userPassword(" . $reset_data . ");'>Password</a></li>
                                            <li role='presentation'><a role='menuitem' tabindex='-1' onclick='smhPPV.statusUser(" . $status_data . ");'>" . $status_text . "</a></li>                                               
                                            <li role='presentation' style='border-top: solid 1px #f0f0f0;'><a role='menuitem' tabindex='-1' onclick='smhPPV.deleteUser(" . $delete_data . ");'>Delete</a></li>
                                        </ul>
                                    </div>
                                </span>";
                ...
                $row[] = "<div class='data-break'><a onclick='smhPPV.viewDetails(\"" . $user['user_details'] . "\");'>View Details <i class='fa fa-external-link' style='width: 100%; text-align: center; display: inline; font-size: 12px;'></i></a></div>";
                $row[] = "<div class='data-break'>" . $restriction . "</div>";
    ...
    public function update_fname($sm_ak, $auth_key, $fname) {
    ...
    public function update_lname($sm_ak, $auth_key, $lname) {
        $success = array('success' => false);
        $pid = $this->smcipher->decrypt($sm_ak);
        $is_logged_in = $this->is_logged_in($auth_key, $sm_ak);
        if ($is_logged_in['success']) {
    ...
    public function update_restriction($pid, $ks, $email, $restriction, $tz) {
        $success = array('success' => false);
    ...
    public function delete_user($pid, $ks, $uid) {
        ...
        if ($valid['success']) {
            ...
            if ($this->accounts->affected_rows() > 0) {
                ...
                $this->accounts->delete('user_details');
                $success = array('success' => true);
            } else {
                $success = array('error' => 'An error occured!');
            }
        } else {
            $success = array('success' => false);
        }
    ...
    public function login_user($un, $pswd, $sm_ak, $entryId) {
        $pid = $this->smcipher->decrypt($sm_ak);
        $success = array('success' => false);
    ...
    public function check_if_active($pid, $un) {
        ...
        foreach ($result as $r) {
            ...
            $has_session = $r['auth_key'];
        }

        if ($logged_in && $has_session) {
        ...
        if ($this->already_activated($pid, $email)) {
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($akey == $activate_key) {
                    ...
                    if ($activate['success']) {
                        $success = $this->create_auth_key($email, $userId, $sm_ak);
                    }
                } else {
                    $success = array('success' => false);
                }
    ...
    public function create_auth_key($un, $uid, $sm_ak) {
        $pid = $this->smcipher->decrypt($sm_ak);
        $success = false;
        ...
        $this->accounts->limit(1);
        if ($this->accounts->affected_rows() > 0) {
            $success = array('success' => true, 'auth_key' => $auth_key, 'user_id' => $uid);
        } else {
            $success = false;
        }
    ...
    public function add_user($pid, $ks, $fname, $lname, $email, $pass, $restriction, $status, $tz) {
        $success = array('success' => false);
    ...
    public function register_user($sm_ak, $fname, $lname, $email, $pass, $tz, $url, $attrs) {
        $pid = $this->smcipher->decrypt($sm_ak);
        $success = array('success' => false);
    ...
    public function email_activation($pid, $sm_ak, $fname, $lname, $email, $activation_key, $url) {
        ...
        $body = 'Hi, ' . $fname . ' ' . $lname . '<br><br>Thank you for registering! To activate your account, please click on the following link: <a href="https://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/verify.php?pid=' . $pid . '&sm_ak=' . $sm_ak . '&akey=' . $activation_key . '&email=' . $email . '&url=' . $url . '">Click Here to Activate</a><br><br>Best Regards,<br>' . $business_name;
    ...
    public function email_reset_email($pid, $email, $sm_ak, $reset_token, $url) {
        ...
        $body = 'Hi, ' . $name . '<br><br>Your email reset request has been successfully submitted. To reset your email, please visit the following link: <a href="https://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/resetEmail.php?pid=' . $pid . '&sm_ak=' . $sm_ak . '&reset_token=' . $reset_token . '&email=' . $email . '&url=' . $url . '">Click Here to Reset Your Email</a><br><br>Best Regards,<br>' . $business_name;
    ...
    public function email_reset_pass($pid, $email, $sm_ak, $reset_token, $url) {
        ...
        $body = 'Hi, ' . $name . '<br><br>Your password reset request has been successfully submitted. To reset your password, please visit the following link: <a href="https://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/forgotPass.php?pid=' . $pid . '&sm_ak=' . $sm_ak . '&reset_token=' . $reset_token . '&email=' . $email . '&url=' . $url . '">Click Here to Reset Your Password</a><br><br>Best Regards,<br>' . $business_name;
