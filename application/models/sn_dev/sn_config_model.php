...
// Open the correct DB connection
    ...
    public function set_youtube_live_config_false($platforms_config) {
        $new_platforms_config = array();
        foreach ($platforms_config as $platform) {
            if ($platform['platform'] == 'facebook_live') {
                if ($platform['status']) {
    ...
    public function set_youtube_vod_config_false($platforms_config) {
        $new_platforms_config = array();
        foreach ($platforms_config as $platform) {
            if ($platform['platform'] == 'facebook') {
                if ($platform['status']) {
    ...
    public function build_vod_sn_config($youtube_video_id, $youtube_upload_status, $facebook_video_id, $facebook_upload_status) {
        $platforms_config = array();
        if ($youtube_video_id) {
            array_push($platforms_config, array('platform' => 'youtube', 'status' => true, 'upload_status' => $youtube_upload_status, 'videoId' => $youtube_video_id));
        } else if (!$youtube_video_id) {
            array_push($platforms_config, array('platform' => 'youtube', 'status' => false));
        }
        if ($facebook_video_id) {
            array_push($platforms_config, array('platform' => 'facebook', 'status' => true, 'upload_status' => $facebook_upload_status, 'videoId' => $facebook_video_id));
...
//                $success = array('success' => true);
//            } else {
//                $success = array('success' => false, 'message' => $process_youtube_upload_a['message']);
//            }
//        } else {
//            $process_youtube_upload_b = $this->process_youtube_upload_b($pid, $eid, $platform, $projection);
//            if ($process_youtube_upload_b['success']) {
//                $success = array('success' => true);
//            } else {
//                $success = array('success' => false, 'message' => $process_youtube_upload_b['message']);
//            }
//        }
    ...
    public function process_youtube_upload_a($pid, $eid, $platform, $projection, $vod_platforms, $partnerData) {
        $success = array('success' => false);
        if ($projection == 'rectangular') {
            $updated_config = $this->insert_into_vod_sn_config('pending', 'uploading', null, null, $vod_platforms['platforms']);
            if ($updated_config['success']) {
    ...
    public function process_youtube_upload_b($pid, $eid, $platform, $projection) {
        $success = array('success' => false);
        if ($projection == 'rectangular') {
            $vod_sn_config = $this->add_vod_sn_config($pid, $eid, $platform, 'pending', 'uploading');
                        ...
                        if ($platform['status']) {
                            ...
                            if ($is_uploading) {
                                ...
                                if (count($vod_platforms['platforms']) > 0) {
                                    ...
                                    $vod_sn_config = $this->build_vod_sn_config(null, null, null, null);
                                    $partnerData = $this->update_sn_partnerData($pid, $eid, $vod_sn_config['sn_config']);
                                    if ($partnerData['success']) {
                                        $success = array('success' => true);
                                    } else {
                                        $success = array('success' => false, 'message' => 'Could not update entry partnerData');
