    ...
    public function get_public_channels($pid, $tz) {
        ...
        if (count($live_channels['data']) > 0) {
            ...
            foreach ($live_channels['data'] as $channel) {
                $live_status = 'Off Air';
                $live_channel_segment = $this->get_live_channel_segment($pid, $ks, $channel['id']);
...
//                }
    ...
    public function get_channel_entries($pid, $ks, $start, $length, $draw, $tz, $cid, $search) {
        ...
        if ($valid['success']) {
            ...
            if ($has_service) {
                $this->config = $this->load->database('kaltura', TRUE);
                $live_channel_segments = $this->get_live_channel_segment_entries($pid, $ks, $cid, $start, $length, $search);
...
//                    $end_date = $end_dt->format('Y-m-d h:i:s A');
    ...
    public function get_live_channel_segment($pid, $ks, $cid) {
        $success = array('success' => false);
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $res) {
                ...
                $entry_details = $this->smportal->get_ott_entry_details($pid, $entry_id);
                if ($entry_details['success']) {
                    $pcid = (int) $res['id'];
                    $id = (int) $res['live_segment_id'];
                    $name = $entry_details['entry_info']['name'];
    ...
    public function get_live_channel_segment_entries($pid, $ks, $cid, $start, $length, $search) {
        $success = array('success' => false);
        $this->config = $this->load->database('ch', TRUE);
    ...
    public function push_schedule($pid, $ks) {
        ...
        if ($schedule['success']) {
            if (count($schedule['schedule']) > 0) {
                $url = 'http://10.5.25.39:1935/ott/update';
                            ...
                            foreach ($programs['nonrepeat_programs'] as $nonrepeat_program) {
                                if ($nonrepeat_program['end_date'] >= $now_date) {
                                    ...
                                    if ($entry_details['success']) {
                                        ...
                                        if ($entry_details['entry_info']['type'] === 7) {
                                            $video_srcs = array();
                                            ...
                                            $new_start_date = $start_date_mod->format('Y-m-d H:i:s');

                                            $video_src = $this->buildVideoSrcs($partner_id, $entry_details['entry_info']['ks'], null, $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], 1, $new_start_date, $now_date, $is_plist, $plist_start);
                                            array_push($video_srcs, $video_src);
                                            $repeat = false;
                                            array_push($playlist, array('name' => 'pl' . $plist_num, 'playOnStream' => $channel, 'repeat' => $repeat, 'scheduled' => $new_start_date, 'video_srcs' => $video_srcs));
                                            $plist_num++;
                                        }
                                    } else {
                                        $sid = (int) $nonrepeat_program['lsid'];
                                        $this->delete_program_int($partner_id, $entry_details['ks'], $sid);
                                    }
                                }
                            }
                        }
                            ...
                            foreach ($programs['repeat_programs'] as $repeat_programs) {
                                ...
                                if (count($rec_programs['date_range_found']) > 0) {
                                    if ($rec_programs['date_range_found']['end_date'] >= $now_date) {
                                        ...
                                        $plist_start = null;
                                        $video_srcs = array();
                                        $entry_details = $this->smportal->get_ott_entry_details($partner_id, $repeat_programs['entry_id']);
                                        if ($entry_details['success']) {
                                            if ($entry_details['entry_info']['type'] === 1) {
                                                $video_src = $this->buildVideoSrcs($partner_id, $entry_details['entry_info']['ks'], $repeat_programs['entry_id'], $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], $repeat_programs['event_length'], $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $plist_start);
                                                array_push($video_srcs, $video_src);
                                                $repeat = false;
                                            } else if ($entry_details['entry_info']['type'] === 7) {
                                                $video_src = $this->buildVideoSrcs($partner_id, $entry_details['entry_info']['ks'], $repeat_programs['entry_id'], $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], $repeat_programs['event_length'], $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $plist_start);
                                                ...
                                                $is_plist = true;
                                                $pexplode = explode(',', $entry_details['entry_info']['playlistContent']);
                                                $remianing_plist_duration = (int) $repeat_programs['event_length'];
                                                if ($now_date >= $rec_programs['date_range_found']['start_date']) {
                                                    $date1 = new DateTime($rec_programs['date_range_found']['start_date']);
                                                    $date2 = new DateTime($now_date);
                                                    $diffInSeconds = $date2->getTimestamp() - $date1->getTimestamp();
                                                    $remianing_plist_duration -= (int) $diffInSeconds;
                                                }
                                                        ...
                                                        if ($eid_details['entry_info']['type'] != 7) {
                                                            if ($diffInSeconds > 0) {
                                                                ...
                                                                if ((int) $diffInSeconds >= (int) $eid_details['entry_info']['duration']) {
                                                                    ...
                                                                    $plist_start = (int) $eid_details['entry_info']['duration'] - (int) $diffInSeconds;
                                                                    $video_src = $this->buildVideoSrcs($partner_id, $entry_details['entry_info']['ks'], $eid, $eid_details['entry_info']['type'], $eid_details['entry_info']['duration'], $remianing_plist_duration, $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $diffInSeconds);
                                                                    array_push($video_srcs, $video_src);
                                                                    $remianing_plist_duration -= (int) $plist_start;
                                                                    $diffInSeconds = 0;
                                                                }
                                                            } else {
                                                                $video_src = $this->buildVideoSrcs($partner_id, $entry_details['entry_info']['ks'], $eid, $eid_details['entry_info']['type'], $eid_details['entry_info']['duration'], $remianing_plist_duration, $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $plist_start);
                                                                array_push($video_srcs, $video_src);
                                                                $remianing_plist_duration -= (int) $eid_details['entry_info']['duration'];
                                                            }
                                                        }
        ...
        if ($entryType === 1) {
            ...
            if ($entryId) {
                ...
                if ($getStreamName['multiBitrate']['status']) {
                    ...
                    $streamName = $getStreamName['multiBitrate']['streamName'];
                }
                        ...
                        if ($nonrepeat_program['end_date'] >= $now_date) {
                            ...
                            if ($entry_details['success']) {
                                ...
                                if ($entry_details['entry_info']['type'] === 7) {
                                    ...
                                    $video_src = $this->buildVideoSrcs($partner_id, $ks, null, $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], 1, $new_start_date, $now_date, $is_plist, $plist_start);
                                    array_push($video_srcs, $video_src);
                                    $repeat = false;
                                    array_push($playlist, array('name' => 'pl' . $plist_num, 'playOnStream' => $channel, 'repeat' => $repeat, 'scheduled' => $new_start_date, 'video_srcs' => $video_srcs));
                                    $plist_num++;
                    ...
                    foreach ($programs['repeat_programs'] as $repeat_programs) {
                        ...
                        if (count($rec_programs['date_range_found']) > 0) {
                            if ($rec_programs['date_range_found']['end_date'] >= $now_date) {
                                ...
                                if ($entry_details['success']) {
                                    if ($entry_details['entry_info']['type'] === 1) {
                                        $video_src = $this->buildVideoSrcs($partner_id, $ks, $repeat_programs['entry_id'], $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], $repeat_programs['event_length'], $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $plist_start);
                                        array_push($video_srcs, $video_src);
                                        $repeat = false;
                                    } else if ($entry_details['entry_info']['type'] === 7) {
                                        $video_src = $this->buildVideoSrcs($partner_id, $ks, $repeat_programs['entry_id'], $entry_details['entry_info']['type'], $entry_details['entry_info']['duration'], $repeat_programs['event_length'], $rec_programs['date_range_found']['start_date'], $now_date, $is_plist, $plist_start);
    ...
    public function remove_live_stream_duplicates($playlist) {
        ...
        foreach ($playlist as $p) {
            foreach ($p['video_srcs'] as $srcs) {
                if ($srcs['video_src'] == '') {
                    ...
                    array_push($live_stoppers, array('index' => $index, 'name' => $live_stopper_name, 'scheduled' => $live_stopper_schedule));
                    //break 2;
                }
            }
            $index++;
        ...
        if ($live_stopper_found) {
            foreach ($live_stoppers as $live_stopper) {
                syslog(LOG_NOTICE, "SMH DEBUG : remove_live_stream_duplicates: live_stopper_found: " . print_r($live_stopper_found, true));
                syslog(LOG_NOTICE, "SMH DEBUG : remove_live_stream_duplicates: live_stopper_name: " . print_r($live_stopper['name'], true));
                syslog(LOG_NOTICE, "SMH DEBUG : remove_live_stream_duplicates: live_stopper_schedule: " . print_r($live_stopper['scheduled'], true));
                syslog(LOG_NOTICE, "SMH DEBUG : remove_live_stream_duplicates: live_stopper_index: " . print_r($live_stopper['index'], true));
            }
        }
        ...
        if ($live_stopper_found) {
            foreach ($playlist as $p) {
                foreach ($live_stoppers as $live_stopper) {
                    if ($live_stopper['name'] != $p['name']) {
                        if ($live_stopper['scheduled'] == $p['scheduled']) {
                            array_push($live_stoppers_indexes, $live_stopper['index']);
                        }
                    }
                }
            }
        ...
        if (count($live_stoppers_indexes) > 0) {
            foreach ($live_stoppers_indexes as $live_index) {
                unset($playlist[$live_index]);
            }
        }
    ...
    public function delete_program_int($pid, $ks, $sid) {
        $success = array('success' => false);
        if (strpos($sid, ',') !== false) {
            $sid_explode = array_map('intval', explode(',', $sid));
            foreach ($sid_explode as $lsid) {
...
//            }
    ...
    public function get_program_dates($pid, $pcid, $cid, $start_date, $end_date) {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $res) {
                if ($res['repeat']) {
                    array_push($programs_repeat, array('lsid' => $res['live_segment_id'], 'start_date' => $res['start_date'], 'end_date' => $res['end_date'], 'rec_type' => $res['rec_type'], 'event_length' => $res['event_length'], 'entry_id' => $res['entry_id']));
                } else {
                    array_push($programs_nonrepeat, array('lsid' => $res['live_segment_id'], 'start_date' => $res['start_date'], 'end_date' => $res['end_date'], 'event_length' => $res['event_length'], 'entry_id' => $res['entry_id']));
                }
            }
