...
class Ppv_entry_model extends CI_Model {

    public function __construct() {
        // Open the correct DB connection
        $this->entry = $this->load->database('ppv', TRUE);
    ...
    public function get_entry_list($pid, $ks, $start, $length, $search, $draw) {
        ...
        if ($valid['success']) {
            $columns = array('status', 'kentry_id', 'kentry_name', 'media_type', 'ticket_ids', 'ac_id', 'created_at', 'updated_at');
...
//            }
            ...
            $count = $this->entry->query('SELECT count(*) AS `Count` FROM entry WHERE partner_id = "' . $valid['pid'] . '"');
            ...
            foreach ($entry_res as $entry) {
                ...
                $edit_data = '\'' . $entry['kentry_id'] . '\',\'' . $entry['kentry_name'] . '\',' . $entry['media_type'] . ',\'' . str_replace(",", "_", $entry['ticket_ids']) . '\',' . $entry['ac_id'] . ',\'' . $entry['ac_name'] . '\'';
                $embed_data = $valid['pid'] . ',\'' . $entry['kentry_id'] . '\',' . $entry['media_type'];

                $tickets_ph = '';
                $tickets = $this->get_ticket_names($pid, $entry['ticket_ids']);

                foreach ($tickets as $ticket) {
                    $tickets_ph .= '<img width="18px" title="' . $ticket . '" data-placement="top" data-toggle="tooltip" data-original-title="' . $ticket . '" rel="tooltip" src="/img/ticket-small-icon.png">&nbsp;';
                }
                ...
                $actions = '<span class="dropdown header">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu" id="menu" role="menu" aria-labelledby="dropdownMenu"> 
                                            <li role="presentation"><a role="menuitem" tabindex="-1" onclick="smhPPV.editEntry(' . $edit_data . ');">Entry</a></li> 
                                            <li role="presentation"><a role="menuitem" tabindex="-1" onclick="smhPPV.previewEmbed(\'' . $sm_ak['sm_ak'] . '\',' . $player_uiconf . ',' . $player_width . ',' . $player_height . ',\'' . $entry['kentry_id'] . '\',\'' . $mode . '\',' . $wrap_width . ');">Preview & Embed</a></li> 
                                            <li role="presentation"><a role="menuitem" tabindex="-1" onclick="smhPPV.statusEntry(' . $status_data . ');">' . $status_text . '</a></li>
                                            <li role="presentation" style="border-top: solid 1px #f0f0f0;"><a role="menuitem" tabindex="-1" onclick="smhPPV.deleteEntry(' . $delete_data . ');">Delete</a></li>
                                        </ul>
                                    </div>
                                </span>';
                ...
                $row[] = "<div class='data-break'>" . $tickets_ph . "</div>";
    ...
    public function get_ticket_names($pid, $ids) {
        $ticket_names = array();
        $ticket_ids = explode(",", $ids);

        foreach ($ticket_ids as $ticket_id) {
            $this->entry->select('ticket_name')
                    ->from('ticket')
                    ->where('partner_id', $pid)
    ...
    public function check_entry_mem($pid, $kentry_id) {
        $success = false;
        $this->entry->select('*')
                ->from('mem_entry')
    ...
    public function update_entry_mem($pid, $kentry_id, $ac_id, $ac_name) {
        $success = array('success' => false);
        $data = array(
            'ac_id' => $ac_id,
            'ac_name' => $ac_name,
    ...
    public function add_entry($pid, $ks, $kentry_id, $kentry_name, $ac_id, $ac_name, $media_type, $ticket_ids, $status) {
        $success = array('success' => false);
        $valid = $this->verfiy_ks($pid, $ks);
        if ($valid['success']) {
            if (!$this->check_entry($valid['pid'], $kentry_id)) {
                $data = array(
                    ...
                    'ticket_ids' => $this->entry->escape_str(trim($ticket_ids)),
                    'created_at' => date("Y-m-d H:i:s"),
                    'status' => $this->entry->escape_str(trim($status))
                );

                $this->entry->insert('entry', $data);
                if ($this->entry->affected_rows() > 0) {
                    if ($this->check_entry_mem($pid, $kentry_id)) {
                        $this->update_entry_mem($pid, $kentry_id, $ac_id, $ac_name);
                    }
                    if ($media_type == 6) {
                        ...
                        foreach ($entries_list as $entry) {
                            ...
                            $query = $this->entry->get();
                            if ($query->num_rows() > 0) {
                                $this->update_ac_id($valid['pid'], $entry['entry_id'], $ac_id);
                            }
                            if ($this->check_entry_mem($pid, $entry['entry_id'])) {
                                $this->update_entry_mem($pid, $entry['entry_id'], $ac_id, $ac_name);
                            }
                        }
                        ...
                        foreach ($content as $c) {
                            ...
                            $query = $this->entry->get();
                            if ($query->num_rows() > 0) {
                                $this->update_ac_id($valid['pid'], $c, $ac_id);
                            }
                            if ($this->check_entry_mem($pid, $c)) {
                                $this->update_entry_mem($pid, $c, $ac_id, $ac_name);
                            }
                        }
                        $res = $this->smportal->update_entry_ac($valid['pid'], $kentry_id, $ac_id, $media_type);
                        if ($res) {
    ...
    public function update_entry($pid, $ks, $kentry_id, $kentry_name, $ac_id, $ac_name, $media_type, $ticket_ids) {
        $success = array('success' => false);
        $valid = $this->verfiy_ks($pid, $ks);
        if ($valid['success']) {
            if ($this->check_entry($valid['pid'], $kentry_id)) {
    ...
    public function update_playlist_entry($pid, $ks, $ac_id, $playlist_id) {
        $success = array('success' => false);
        $valid = $this->verfiy_ks($pid, $ks);
        if ($valid['success']) {
    ...
    public function update_ac_id($pid, $entry_id, $ac_id) {
        $success = false;
        $data = array(
            'ac_id' => $ac_id,
            'updated_at' => date("Y-m-d H:i:s")
