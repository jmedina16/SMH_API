...
class Ppv_orders_model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->orders = $this->load->database('ppv', TRUE);
...
//            }
    ...
    public function refundPurchasePayPal($pid, $transactionId) {
        ...
        $response = $this->parseString($this->run_pp_curl(PAYPAL_URL, $postVals));
        return $response;
    }
...
//            'item_name' => $itemName,
    ...
    public function get_confirm($sm_ak, $kentry, $ticket_id, $type, $protocol, $has_start) {
        ...
        foreach ($ticket as $t) {
            ...
            $img = '';
            if ($type == 's') {
                $img = '<img src = "' . $protocol . '://ecimages.streamingmediahosting.com/p/' . $pid . '/thumbnail/entry_id/' . $kentry . '/width/169" width="169px" />';
            } else if ($type == 'p') {
                $entry_id = $this->smportal->get_thumb($pid, $kentry);
                $img = '<img src = "' . $protocol . '://ecimages.streamingmediahosting.com/p/' . $pid . '/thumbnail/entry_id/' . $entry_id . '/width/169" width="169px" />';
            } else if ($type == 'cl' || $type == 'cr' || $type == 'ct' || $type == 'cb') {
                $entry = $this->smportal->get_cat_thumb($pid, $kentry);
                $img = '<img src = "' . $protocol . '://ecimages.streamingmediahosting.com/p/' . $pid . '/thumbnail/entry_id/' . $entry . '/width/169" width="169px" />';
            }
