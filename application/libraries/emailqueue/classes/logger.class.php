...
class logger {
    ...
    function logger() {
        ...
        $this->filename_smtp = APP_DIR . LOGS_DIR . "/" . $this->get_log_filename("smtp");
    ...
    function add_log_smtp($data) {
        $handle = fopen($this->filename_smtp, "a");
        if (fwrite($handle, $data) === FALSE) {
            echo "Error: can't write to " . $this->filename_smtp;
            die;
        }
        fclose($handle);
    }
